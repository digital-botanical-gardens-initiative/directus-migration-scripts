...
directus_collection = f"{directus_instance}/items/Mass_Spectrometry_Analysis/?limit=-1"
...
new_column_names = {
    "UUID_mass_spec": "uuid_ms_file",
    "mass_spec_id": "filename",
    "status": "status",
    # "aliquot_id": "parent_sample_container",
    "injection_volume": "injection_volume",
    "injection_method": "injection_method",
    "ms_id": "instrument_used",
    "converted": "converted",
    "added": "processed",
}
...
df["injection_volume_unit"] = 18
df["instrument_used"] = 1
...
df["parent_sample_container"] = df.apply(assign_values_blk_al, axis=1)
...
df["parent_sample_container"] = df["aliquot_id"].apply(get_primary_key_parent)


# Function to get injection methods primary keys
def get_primary_key_inj_met(sample_code: str) -> Optional[int]:
    params = {
    ...
    # Create a session object for making requests
    session = requests.Session()
    response = session.get("https://emi-collection.unifr.ch/directus-test/items/Injection_Methods/", params=params)
    if response.status_code == 200:
        data = response.json()
        if data["data"]:
            return int(data["data"][0]["id"])
...
# Apply the function to retrieve primary keys and store them in a new column
df["injection_method"] = df["injection_method"].apply(get_primary_key_inj_met)
...
if response.status_code == 200:
    ...
    directus_collection = f"{directus_instance}/items/MS_Data/"
